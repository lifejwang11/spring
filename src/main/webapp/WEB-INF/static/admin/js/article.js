...
function subArticle(status) {
    ...
    console.log(params);
    var url = $('#articleForm #cid').val() != '' ? 'http://localhost:5000/blogsapi/admin/article/modify' : 'http://localhost:5000/blogsapi/admin/article/publish';
    tale.post({
        url:url,
        data:params,
        success: function (result) {
            if (result && result.code == 'success') {
                tale.alertOk({
                    ...
                    then: function () {
                        setTimeout(function () {
                            window.location.href = 'http://localhost:5000/blogsapi/admin/article';
                        }, 500);
                    }
                });
            } else {
